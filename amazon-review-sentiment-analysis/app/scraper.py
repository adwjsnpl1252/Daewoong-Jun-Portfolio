...
def scrape_webpage(temp_url):
    ...
    while flag == 0 and page_num <= 1000:
        try:
            ...
            for item in reviews:
                ...
                # except:
                #     vote = 0
                verified = item.find('span', {'data-hook': 'avp-badge'}).text.strip()
    ...
    for i in range(len(review_titles)):
        review_title_lang = lang_detector.predict(review_titles[i].strip(), k=3)
    ...
    review_titles = [ review_titles[i] for i in range(len(review_titles)) if i not in titles_to_remove]
    review_bodies = [ review_bodies[i] for i in range(len(review_bodies)) if i not in bodies_to_remove]
